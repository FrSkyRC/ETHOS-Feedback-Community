                        ...
                        if field.type == 9 then
                                ...
                                if field.widget == nil then
                                        ...
                                        field.widget = form.addChoiceField(line, nil, field.values, function()
                                                ...
                                                field.value = value
                                                elrs.pushFrame(0x2D, {deviceId, handsetId, field.id, value})
                                        end)
                                end
                                                        ...
                                                        field.dialog:buttons({
                                                                {
                                                                        label = "OK",
                                                                        action = function()
                                                                                elrs.pushFrame(0x2D, {deviceId, handsetId, field.id, 4}) -- lcsConfirmed
                                ...
                                elseif field.widget == nil then
                                        ...
                                        field.widget = form.addTextButton(line, nil, field.name, function()
                                                if field.status < 4 then
                                                        field.status = 1
                                                        elrs.pushFrame(0x2D, {deviceId, handsetId, field.id, field.status})
                                                        fieldPopup = field
...
function elrs.wakeup(widget)
        ...
        if fieldPopup then        
                if time > fieldTime and fieldPopup.status ~= 3 then
                        ...
                        elrs.pushFrame(0x2D, {deviceId, handsetId, fieldPopup.id, 6}) -- lcsQuery
                        fieldTime = time + fieldPopup.timeout / 100
                end
        elseif time > devicesRefreshTime and deviceId == nil then

                devicesRefreshTime = time + 1 -- 1s
                elrs.pushFrame(0x28, {0x00, 0xEA})

        elseif time > fieldTime and deviceId ~= nil then
                if #loadQ > 0 then
                             
                        elrs.pushFrame(0x2C, {deviceId, handsetId, loadQ[#loadQ], fieldChunk})
                        fieldTime = time + 0.5
                end
        end
...
function elrs.event(widget, category, value, x, y)
