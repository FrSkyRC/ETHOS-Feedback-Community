...
local CommonFile = assert(loadfile(RB35GlobalPath .. "common.lua"))()
...
STR = assert(loadfile(RB35GlobalPath .. "i18n/i18n.lua"))().translate
...
local REMOTE_DEVICE = {address = 0xFE, state = STATE_READ, field = nil, label = STR("RemoteDevice"), dataHandler = function (value, task)
  ...
  local FrSkyProducts = assert(loadfile(RB35GlobalPath .. "products.lua"))()
...
local pages = {{file = assert(loadfile(RB35GlobalPath .. "basic/basic.lua")()), label = STR("BasicConfig")},
               {
                 name = STR("StabilizerGroup1"),
                 subPages = {
                   {file = assert(loadfile(RB35GlobalPath .. "group1/precali1.lua")()), label = STR("Calibration")},
                   {file = assert(loadfile(RB35GlobalPath .. "group1/stab1.lua")()), label = STR("Configuration")},}
                 },
               {
                 name = STR("StabilizerGroup2"),
                 subPages = {
                   {file = assert(loadfile(RB35GlobalPath .. "group2/precali2.lua")()), label = STR("Calibration")},
                   {file = assert(loadfile(RB35GlobalPath .. "group2/stab2.lua")()), label = STR("Configuration")},}
                 },
               {file = assert(loadfile(RB35GlobalPath .. "cali/cali.lua")()), label = STR("SixAxisCali")}}
